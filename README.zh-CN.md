...
`bench.json` 是一个 `BenchConfig` JSON 对象——字段说明见下面的[配置字段说明](#配置字段说明)。退出码只反映"这次压测本身有没有跑完"：配置文件读不出来/格式不对是 `2`，一个响应都没拿到是 `1`；哪怕成功率是 0%，只要拿到了报告，退出码依然是 `0`（成功率会体现在 JSON 里）。跑的是和 GUI 完全相同的压测引擎，两边结果不会出现分歧。

要在 CI 里按性能阈值判定，把一次跑出来的报告跟基准对比：

```bash
inferscope compare baseline.json report.json --max-regression 10%
```

会检查 TTFT/TPOT/E2E 的每个百分位，加上平均吞吐量和成功率；只要有一项相对基准的变化超过设定百分比（延迟类指标变慢、吞吐量/成功率变低都算），退出码就是 `1`，否则是 `0`。默认打印人类可读的表格，加 `--json` 可以输出机器可读格式。
